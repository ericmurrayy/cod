...
def compile_page(src_rel):
    ...
    head = (
        ...
        + f'<link rel="stylesheet" href="{prefix}assets/site.css?v={ASSET_V}">\n'
        + f'<script src="{prefix}assets/site.js?v={ASSET_V}" defer></script>\n'
        + "</head>\n<body>\n"
...
# Bump on any assets/site.css or site.js change so cached HTML (max-age 600
# on GitHub Pages) can't keep serving a stale stylesheet after a deploy.
ASSET_V = "4"
...
/* Never let one long unbreakable word (an email address, "Tyngsborough.")
   push the page wider than the viewport: allow in-word breaks in headings,
   and let grid/flex tracks shrink below the word's natural width. */
